...
bool SDstartFileTransfer(char *filename, uint8_t filenamesize)
{
  ...
  do
  {
    ...
    SDTXPacketL = LoRa.transmitDTIRQ(SDDTheader, DTFileOpenHeaderL, (uint8_t *) filename, filenamesize, NetworkID, TXtimeoutmS, TXpower,  WAIT_TX);
    if (SDDTLED >= 0)
    {
    ...
    ValidACK = LoRa.waitACKDTIRQ(SDDTheader, DTFileOpenHeaderL, ACKopentimeoutmS);
...
bool SDsendFileSegment(uint16_t segnum, uint8_t segmentsize)
{
  ...
  do
  {
    ...
    SDTXPacketL = LoRa.transmitDTIRQ(SDDTheader, DTSegmentWriteHeaderL, (uint8_t *) SDDTdata, segmentsize, NetworkID, TXtimeoutmS, TXpower,  WAIT_TX);
    if (SDDTLED >= 0)
    {
      digitalWrite(SDDTLED, LOW);
    }
    ...
    ValidACK = LoRa.waitACKDTIRQ(SDDTheader, DTSegmentWriteHeaderL, ACKsegtimeoutmS);
    SDRXPacketType = SDDTheader[0];
...
bool SDendFileTransfer(char *filename, uint8_t filenamesize)
{
  ...
  do
  {
    ...
    SDTXPacketL = LoRa.transmitDTIRQ(SDDTheader, DTFileCloseHeaderL, (uint8_t *) filename, filenamesize, NetworkID, TXtimeoutmS, TXpower,  WAIT_TX);
    if (SDDTLED >= 0)
    {
      digitalWrite(SDDTLED, LOW);
    }
    ...
    ValidACK = LoRa.waitACKDTIRQ(SDDTheader, DTFileCloseHeaderL, ACKclosetimeoutmS);
    SDRXPacketType = SDDTheader[0];
...
bool SDsendDTInfo()
{
  ...
  do
  {
    ...
    Monitorport.println(localattempts);
#endif
    SDTXPacketL = LoRa.transmitDTIRQ(SDDTheader, DTInfoHeaderL, (uint8_t *) SDDTdata, 0, NetworkID, TXtimeoutmS, TXpower,  WAIT_TX);
    ...
    ValidACK = LoRa.waitACKDTIRQ(SDDTheader, DTInfoHeaderL, ACKsegtimeoutmS);
...
bool SDreceiveaPacketDT()
{
  ...
  SDRXPacketType = 0;
  SDRXPacketL = LoRa.receiveDTIRQ(SDDTheader, HeaderSizeMax, (uint8_t *) SDDTdata, DataSizeMax, NetworkID, RXtimeoutmS, WAIT_RX);
...
bool SDprocessSegmentWrite()
{
  ...
  if (!SDDTFileOpened)
  {
    ...
    LoRa.sendACKDTIRQ(SDDTheader, DTStartHeaderL, TXpower);
    if (SDDTLED >= 0)
    {
      digitalWrite(SDDTLED, LOW);
    }
  ...
  if (SDDTSegment == SDDTSegmentNext)
  {
    ...
    LoRa.sendACKDTIRQ(SDDTheader, DTSegmentWriteHeaderL, TXpower);
...
bool SDprocessFileOpen(uint8_t *filename, uint8_t filenamesize)
{
  ...
  LoRa.sendACKDTIRQ(SDDTheader, DTFileOpenHeaderL, TXpower);
  if (SDDTLED >= 0)
  {
    digitalWrite(SDDTLED, LOW);
  }
...
bool SDprocessFileClose()
{
  ...
  LoRa.sendACKDTIRQ(SDDTheader, DTFileCloseHeaderL, TXpower);
  if (SDDTLED >= 0)
  {
    digitalWrite(SDDTLED, LOW);
  }
